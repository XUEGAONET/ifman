...
**请勿同时运行多个ifman，程序目前还未支持自动检测防止多实例运行。**
...
**如下配置仅为简单介绍使用，如需example，请查看config目录下的文件；如需详细的字段描述，请继续向下看**

```yaml
logger:
  # 支持模式：none/rotate/single/stdout
  mode: single
...
| 字段名称  | 内容释义   | 数据类型 | 其他                                                       |
| --------- | ---------- | -------- | ---------------------------------------------------------- |
| ttl       | TTL        | uint8    | 可能会和NoPmtuDisc冲突，暂时还未测试。为空则由系统自动生成 |
